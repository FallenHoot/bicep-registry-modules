...
// --- Retention Settings ---
@description('Optional. Number of days of data to retain in the msexports container. Default: 0 (delete after ingestion).')
...
@description('Optional. Number of days of data to retain in the Data Explorer *_raw tables. Default: 0 (delete after normalization).')
...
// --- Fabric Capacity ---
@description('Optional. Number of capacity units for the Microsoft Fabric capacity. This is the number in your Fabric SKU (e.g., Trial = 1, F2 = 2, F64 = 64). Used to manage parallelization in data pipelines. Default: 2.')
@minValue(1)
@maxValue(2048)
#disable-next-line no-unused-params // Passed to ADF pipeline parallelization settings in future
param fabricCapacityUnits int = 2

// --- Managed Exports Override ---
@description('Optional. Enable managed exports where your FinOps hub instance will create and run Cost Management exports on your behalf. When true, requires billing scopes to be configured. Default: true.')
param enableManagedExportsOverride bool = true
...
// Managed exports: explicit override intersected with billing type support
var enableManagedExports = enableManagedExportsOverride && contains(['ea', 'mca', 'mpa', 'auto'], billingAccountType)
...
module adxSchemaSetup 'modules/adxSchemaSetup.bicep' = if (deployAdxSchema) {
  ...
  params: {
    ...
    rawRetentionInDays: dataExplorerRawRetentionInDays > 0 ? dataExplorerRawRetentionInDays : 30
...
// Storage Blob Data Contributor for the managed identity (required for config deployment script + trigger management)
module identityStorageRoleAssignment 'br/public:avm/ptn/authorization/resource-role-assignment:0.1.2' = {
...
// --- Config Deployment Script (settings.json) ---
// Writes/updates settings.json to the config container, matching upstream FinOps toolkit.
// ADF pipelines read this file at runtime for scopes, retention, and version info.
...
module hubDeploymentScript 'modules/hub-deploymentScript.bicep' = {
  ...
  params: {
    storageAccountName: storageAccount.outputs.name
    ftkVersion: ftkVersion
    scopesPipeDelimited: scopesPipeDelimited
...
// Data Explorer database outputs
@description('Name of the Data Explorer database used for ingesting data.')
output ingestionDbName string = deployAdxSchema ? 'Ingestion' : ''

@description('Name of the Data Explorer database used for querying data.')
output hubDbName string = deployAdxSchema ? 'Hub' : ''

// Config deployment script outputs
@description('Indicates whether settings.json was deployed to the config container.')
output configDeployed bool = true
...
output settingsJson object = {
  ...
  retention: {
    msexports: {
      days: exportRetentionInDays
    }
    ingestion: {
      months: ingestionRetentionInMonths
    }
    raw: {
      days: dataExplorerRawRetentionInDays
    }
    final: {
      months: dataExplorerFinalRetentionInMonths
    }
  }
  deployment: {
