...
.DESCRIPTION
    ...
    v3.0 Features (dashboard-aligned):
    - ALL columns referenced by every dashboard KQL query
    ...
    - x_EffectiveUnitPrice, x_BilledUnitPrice for discount analysis
    - x_SkuDescription, x_SkuInstanceType for SKU cost analysis
    - x_OnDemandCost, x_OnDemandUnitPrice for savings calculations
    - Tag coverage variation (~20% untagged for maturity scorecard)
    - Data quality anomaly rows for validation page
...
$ProviderConfigs = @{
    Azure = @{
        ...
        Services = @(
            ...
            @{ Name = "Cosmos DB"; Category = "Databases"; Subcategory = "NoSQL Databases"; Weight = 8; CostMin = 20; CostMax = 500; PricingUnit = "100 RUs"; ConsumedUnit = "Request Units"; PricingBlockSize = 100; PricingUnitDescription = "100 Request Units" },
            @{ Name = "Storage"; Category = "Storage"; Subcategory = "Object Storage"; Weight = 10; CostMin = 5; CostMax = 200; PricingUnit = "1 GB"; ConsumedUnit = "GB"; PricingBlockSize = 1; PricingUnitDescription = "1 GB/Month" },
            @{ Name = "Azure Data Explorer"; Category = "Analytics"; Subcategory = "Analytics Platforms"; Weight = 5; CostMin = 50; CostMax = 600; PricingUnit = "1 Hour"; ConsumedUnit = "Hours"; PricingBlockSize = 1; PricingUnitDescription = "1 Hour" },
            @{ Name = "App Service"; Category = "Compute"; Subcategory = "Web Apps"; Weight = 4; CostMin = 10; CostMax = 300; PricingUnit = "1 Hour"; ConsumedUnit = "Hours"; PricingBlockSize = 1; PricingUnitDescription = "1 Hour" },
            @{ Name = "Key Vault"; Category = "Security"; Subcategory = "Key Management"; Weight = 2; CostMin = 1; CostMax = 50; PricingUnit = "10K Operations"; ConsumedUnit = "10K Operations"; PricingBlockSize = 10000; PricingUnitDescription = "10,000 Operations" },
            @{ Name = "Virtual Network"; Category = "Networking"; Subcategory = "Virtual Networks"; Weight = 2; CostMin = 5; CostMax = 100; PricingUnit = "1 GB"; ConsumedUnit = "GB"; PricingBlockSize = 1; PricingUnitDescription = "1 GB" },
            @{ Name = "Azure Functions"; Category = "Compute"; Subcategory = "Serverless Compute"; Weight = 2; CostMin = 0.10; CostMax = 30; PricingUnit = "1M Executions"; ConsumedUnit = "1M Executions"; PricingBlockSize = 1000000; PricingUnitDescription = "1,000,000 Executions" },
            @{ Name = "Azure Marketplace"; Category = "Marketplace"; Subcategory = "Third Party"; Weight = 5; CostMin = 10; CostMax = 500; PricingUnit = "1 Hour"; ConsumedUnit = "Hours"; PricingBlockSize = 1; PricingUnitDescription = "1 Hour"; IsMarketplace = $true }
        )
        ...
        # VM SKU pool with CPU architecture patterns for x_SkuMeterName
        # Intel: [0-9]+[^ap] patterns like D4sv5, E8sv5
        # AMD:   [0-9]+a patterns like D4asv5, E8asv5
        # Arm64: [0-9]+p patterns like D4psv5, E8psv5
        VmSkus = @(
    ...
    AWS = @{
        ...
        Services = @(
            ...
            @{ Name = "Amazon DynamoDB"; Category = "Databases"; Subcategory = "NoSQL Databases"; Weight = 6; CostMin = 10; CostMax = 400; PricingUnit = "RCUs"; ConsumedUnit = "Read Capacity Units"; PricingBlockSize = 1; PricingUnitDescription = "1 Read Capacity Unit" },
            ...
            @{ Name = "AWS Lambda"; Category = "Compute"; Subcategory = "Serverless Compute"; Weight = 3; CostMin = 0.10; CostMax = 30; PricingUnit = "Requests"; ConsumedUnit = "1M Requests"; PricingBlockSize = 1000000; PricingUnitDescription = "1,000,000 Requests" },
            @{ Name = "Amazon SQS"; Category = "Integration"; Subcategory = "Messaging"; Weight = 2; CostMin = 0.50; CostMax = 20; PricingUnit = "Requests"; ConsumedUnit = "1M Requests"; PricingBlockSize = 1000000; PricingUnitDescription = "1,000,000 Requests" }
        )
    ...
    GCP = @{
        ...
        Services = @(
            ...
            @{ Name = "Cloud Spanner"; Category = "Databases"; Subcategory = "Distributed Databases"; Weight = 5; CostMin = 50; CostMax = 500; PricingUnit = "Node-Hours"; ConsumedUnit = "Node Hours"; PricingBlockSize = 1; PricingUnitDescription = "1 Node-Hour" },
            @{ Name = "Cloud Run"; Category = "Compute"; Subcategory = "Serverless Containers"; Weight = 3; CostMin = 5; CostMax = 100; PricingUnit = "vCPU-Seconds"; ConsumedUnit = "vCPU Seconds"; PricingBlockSize = 1; PricingUnitDescription = "1 vCPU-Second" },
            @{ Name = "Cloud Functions"; Category = "Compute"; Subcategory = "Serverless Compute"; Weight = 3; CostMin = 0.10; CostMax = 30; PricingUnit = "Invocations"; ConsumedUnit = "1M Invocations"; PricingBlockSize = 1000000; PricingUnitDescription = "1,000,000 Invocations" }
        )
    ...
    DataCenter = @{
        ...
        Services = @(
            @{ Name = "Physical Servers"; Category = "Compute"; Subcategory = "Bare Metal"; Weight = 30; CostMin = 200; CostMax = 5000; PricingUnit = "Hours"; ConsumedUnit = "Hours"; PricingBlockSize = 1; PricingUnitDescription = "1 Hour" },
            ...
            @{ Name = "Oracle Database"; Category = "Databases"; Subcategory = "Relational Databases"; Weight = 15; CostMin = 500; CostMax = 10000; PricingUnit = "Processor Licenses"; ConsumedUnit = "Processor Licenses"; PricingBlockSize = 1; PricingUnitDescription = "1 Processor License" },
...
function New-ProviderIdentities {
    ...
    for ($i = 1; $i -le $resourceCount; $i++) {
        ...
        # VM SKU assignment (Azure VMs get specific instance types)
        $vmSku = $null
        if ($Provider -eq "Azure" -and $resourceType -eq "microsoft.compute/virtualmachines" -and $Config.VmSkus) {
    ...
    for ($i = 1; $i -le $commitmentCount; $i++) {
        ...
        $skuTerm = Get-RandomElement -Array @(12, 36)  # 1 year or 3 years in months
...
function New-FocusRow {
    ...
    $contractedCost = [math]::Round($listCost * (100 - $negotiatedDiscountPct) / 100, 10)
    ...
    if ($IncludeCommitments -and $Provider -ne "DataCenter" -and $chargeCategory -eq "Usage" -and
        ...
        if ((Get-Random -Maximum 100) -lt 85) {
            ...
            $effectiveCost = [math]::Round($contractedCost * 0.40, 10)  # 60% savings on contracted
            $billedCost = 0  # Prepaid
        } else {
            $commitmentDiscountStatus = "Unused"
            $effectiveCost = [math]::Round($contractedCost * 0.60, 10)  # Wasted commitment
    ...
    if ($chargeCategory -eq "Usage" -and $null -eq $commitmentDiscountId -and $service.Name -in $spotEligibleServices) {
        if ((Get-Random -Maximum 100) -lt 15) {
            $pricingCategory = "Dynamic"  # FOCUS spec uses "Dynamic", not "Spot"
    ...
    if ($Provider -eq "Azure") {
        # Set VM SKU details for all Azure compute resources
        if ($res.VmSku) {
            $vmSku = $res.VmSku
            ...
            $x_SkuMeterName = $vmSku.MeterName  # Contains architecture pattern (D4s, D4as, D4ps)
    ...
    $effectiveUnitPrice = if ($pricingQuantity -ne 0) { [math]::Round($effectiveCost / $pricingQuantity, 10) } else { 0 }
    $billedUnitPrice = if ($pricingQuantity -ne 0) { [math]::Round($billedCost / $pricingQuantity, 10) } else { 0 }
    $onDemandUnitPrice = if ($pricingQuantity -ne 0) { [math]::Round($onDemandCost / $pricingQuantity, 10) } else { 0 }

    # === DATA QUALITY ANOMALIES (~2% of rows) ===
    $x_SourceChanges = $null
    $qualityRoll = Get-Random -Maximum 100
    if ($qualityRoll -eq 0) {
        # Effective > Contracted (anomaly)
        $effectiveCost = [math]::Round($contractedCost * 1.1, 10)
        $x_SourceChanges = "CostAdjustment"
    } elseif ($qualityRoll -eq 1) {
        # Contracted > List (anomaly)
        $contractedCost = [math]::Round($listCost * 1.05, 10)
    ...
    return [PSCustomObject]@{
        ...
        # SKU columns (dashboard SKU cost analysis, CPU architecture, AHB)
        ...
        # Commitment linkage (dashboard invoicing/utilization)
        x_SkuOrderId                  = $x_SkuOrderId
        x_SkuTerm                     = $x_SkuTerm

        # Pricing detail (dashboard data quality page)
...
Write-Host "FinOps Hub Multi-Cloud FOCUS Test Data Generator v3.0" -ForegroundColor Cyan
Write-Host ("=" * 70) -ForegroundColor Cyan
Write-Host ""
...
foreach ($provider in $providers) {
    ...
    # Write Python script to temp file (avoids heredoc piping issues with pwsh -File)
    $pyTempFile = [System.IO.Path]::GetTempFileName() -replace '\.tmp$', '.py'
    $pythonScript = @"
import pandas as pd, sys
try:
    df = pd.read_csv('$rawCsvPy', low_memory=False)
    sf = $scaleFactor
    cost_cols = ['BilledCost','ContractedCost','EffectiveCost','ListCost',
                 'ContractedUnitPrice','ListUnitPrice',
                 'x_EffectiveUnitPrice','x_BilledUnitPrice',
                 'x_OnDemandCost','x_OnDemandUnitPrice']
    ...
    if fmt in ('Parquet','Both'):
        ...
        print('PARQUET_OK')
    if fmt == 'Parquet' and 'PARQUET_OK' not in locals().get('_printed', ''):
        df.to_parquet('$parquetOutPy', engine='pyarrow', compression='snappy', index=False)
        print('PARQUET_OK')
    print('DONE')
    ...
    sys.exit(1)
"@
    [System.IO.File]::WriteAllText($pyTempFile, $pythonScript)
    $result = (python $pyTempFile 2>&1) -join "`n"
...
Write-Host ""
Write-Host "Dashboard Coverage:" -ForegroundColor Cyan
...
Write-Host "  Marketplace: x_PublisherCategory = 'Marketplace'"
Write-Host "  Data quality anomalies: ~2% rows with cost relationship issues"
Write-Host "  Negotiated discounts: ~60% rows with ListCost > ContractedCost"
Write-Host "  x_EffectiveUnitPrice, x_BilledUnitPrice, x_OnDemandCost/UnitPrice"
Write-Host "  x_SkuDescription, x_SkuInstanceType, x_SkuCoreCount"
Write-Host "  x_BillingAccountAgreement, x_PricingBlockSize, x_PricingUnitDescription"
Write-Host ""
...
Write-Host "  - PricingCategory: Standard (on-demand), Dynamic (spot), Committed (reserved)"
Write-Host ""
